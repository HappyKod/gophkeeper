...
func TestMemoryStorage_GetSecret(t *testing.T) {
	...
	secret, err := s.GetSecret(context.Background(), "user1")
...
func TestMemoryStorage_DeleteSecret(t *testing.T) {
	...
	err = s.DeleteSecret(context.Background(), "user1")
	assert.NoError(t, err)
}
...
func TestSyncSecret(t *testing.T) {
	...
	secret1 := models.Secret{ID: 1, OwnerID: "user1", Value: []byte("secret1"), Type: "type1", Description: "desc1", IsDeleted: false, Ver: time.Now()}
	secret2 := models.Secret{ID: 2, OwnerID: "user1", Value: []byte("secret2"), Type: "type2", Description: "desc2", IsDeleted: true, Ver: time.Now()}
	secret3 := models.Secret{ID: 3, OwnerID: "user2", Value: []byte("secret3"), Type: "type1", Description: "desc3", IsDeleted: false, Ver: time.Now()}
	...
	tests := []struct {
		...
		{"user1", []models.LiteSecret{
			{ID: 1, ValueHash: utils.GetMD5Hash([]byte("secret1")), DescriptionHash: utils.GetMD5Hash([]byte("desc1")), IsDeleted: false, Ver: secret1.Ver},
			{ID: 2, ValueHash: utils.GetMD5Hash([]byte("secret2")), DescriptionHash: utils.GetMD5Hash([]byte("desc2")), IsDeleted: true, Ver: secret2.Ver},
		}},
		{"user2", []models.LiteSecret{
			{ID: 3, ValueHash: utils.GetMD5Hash([]byte("secret3")), DescriptionHash: utils.GetMD5Hash([]byte("desc3")), IsDeleted: false, Ver: secret3.Ver},
		}},
	}
